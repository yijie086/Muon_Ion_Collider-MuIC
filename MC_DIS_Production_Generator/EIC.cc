...
int main() {
  ...
  TFile * f = TFile::Open("EIC111.root","recreate");
...
for (int iQ=0;iQ<=0;iQ++) {
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
    ...
    for (int i = 7; i < pythia.event.size(); ++i){
      if (pythia.event[i].isFinal()&&((pythia.event[i].id()!=11)||(pythia.event[i].id()==11&&pythia.event[pythia.event[i].mother1()].id()!=11))){
  ...
  auto c1d=new TCanvas();
  c1d->SetLogy();
  h1d->Draw();
  c1d->SaveAs("Jetnum.pdf");
  
  auto c1dPar=new TCanvas();
  c1dPar->SetLogy();
  h1dParnum->Draw();
  c1dPar->SaveAs("Parnum.pdf");
  
  auto c1dR=new TCanvas();
  c1dR->SetLogy();
  h1dR->Draw();
  c1dR->SaveAs("JetdR.pdf");
  
  auto c1dPhi=new TCanvas();
  //c1dPhi->SetLogy();
  h1dPhi->Draw();
  c1dPhi->SaveAs("JetdPhi.pdf");
  
  auto c=new TCanvas();
  c->SetLogz();
  gStyle->SetPalette(kBird);
  h2d->Draw("COLZ");
  c->SaveAs("PEre2PEp.pdf");
  
  auto cx=new TCanvas();
  cx->SetLogz();
  gStyle->SetPalette(kBird);
  h2dx->Draw("COLZ");
  cx->SaveAs("Pxre2Pxp.pdf");
  
  auto cy=new TCanvas();
  cy->SetLogz();
  gStyle->SetPalette(kBird);
  h2dy->Draw("COLZ");
  cy->SaveAs("Pyre2Pyp.pdf");
  
  auto cz=new TCanvas();
  cz->SetLogz();
  gStyle->SetPalette(kBird);
  h2dz->Draw("COLZ");
  cz->SaveAs("Pzre2Pzp.pdf");
  ...
  gStyle->SetPalette(kBird);
  h2d3->Draw("COLZ");
  c2->SaveAs("eta2eta.pdf");
  
  auto c3=new TCanvas();
  c3->SetLogz();
  gStyle->SetPalette(kBird);
  h2d4->Draw("COLZ");
  c3->SaveAs("UEpt2phi.pdf");
  
  auto c4=new TCanvas();
  //c4->SetLogz();
  gStyle->SetPalette(kBird);
  h1dPhiej->Draw("COLZ");
  c4->SaveAs("dPhiej.pdf");
  
  
  float muoutmin=1;
  float muoutmax=100*zoombeta;
  
  auto cmueta2E=new TCanvas();
  cmueta2E->SetLogz();
  //gPad->SetLogy();
  gStyle->SetPalette(kBird);
  h2dformuouteta2e->SetMaximum(muoutmax);
  h2dformuouteta2e->SetMinimum(muoutmin);
  h2dformuouteta2e->Draw("COLZ");
  cmueta2E->SaveAs("muk/muouteta2e.pdf");
  
  auto jcmueta2E=new TCanvas();
  jcmueta2E->SetLogz();
  //gPad->SetLogy();
  gStyle->SetPalette(kBird);
  jh2dformuouteta2e->SetMaximum(muoutmax);
  jh2dformuouteta2e->SetMinimum(muoutmin);
  jh2dformuouteta2e->Draw("COLZ");
  jcmueta2E->SaveAs("jetk/jmuouteta2e.pdf");
