...
void readout_jet() {
	...
	ofstream phifile;
	phifile.open("jetKfull.csv",ios::out|ios::trunc);
	t->SetBranchAddress("genJetEta",&do_eta);
	...
	t->SetBranchAddress("genJetPt",&do_pt);
	for(int i=0;i<t->GetEntries();i++){
		t->GetEntry(i);
	...
	t->SetBranchAddress("genJetQ2",&do_Q2);
	for(int i=0;i<t->GetEntries();i++){
		t->GetEntry(i);
	...
	t->SetBranchAddress("genJetx",&do_x);
	for(int i=0;i<t->GetEntries();i++){
		t->GetEntry(i);
