...
void readout_photon() {
	...
	ofstream phifile;
	phifile.open("photonKppp.csv",ios::out|ios::trunc);
	t->SetBranchAddress("gamma_eta",&do_eta);
	...
	t->SetBranchAddress("gamma_pt",&do_pt);
	for(int i=0;i<t->GetEntries();i++){
		t->GetEntry(i);
	...
	t->SetBranchAddress("gamma_Q2",&do_Q2);
	for(int i=0;i<t->GetEntries();i++){
		t->GetEntry(i);
	...
	t->SetBranchAddress("gamma_x",&do_x);
	for(int i=0;i<t->GetEntries();i++){
		t->GetEntry(i);
